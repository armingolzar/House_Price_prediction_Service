...
def Data_Prepration(path):
    ...
    data_train, data_test, label_train, label_test = train_test_split(
        house_dataset_data, label, test_size=0.2, random_state=42, shuffle=True
    )

    # data_train category
    data_train_numerical = data_train.loc[
        :, ["area", "bedrooms", "bathrooms", "stories", "parking"]
    ]

    data_train_categorical = data_train.loc[
        :,
        [
            "mainroad",
            "guestroom",
    ...
    # data_test category
    data_test_numerical = data_test.loc[
        :, ["area", "bedrooms", "bathrooms", "stories", "parking"]
    ]

    data_test_categorical = data_test.loc[
        :,
        [
            "mainroad",
            "guestroom",
    ...
    data_train_numerical_scaled = pd.DataFrame(
        data_train_numerical_scaled, columns=data_train_numerical.columns
    )
    data_test_numerical_scaled = SS.transform(data_test_numerical)
    data_test_numerical_scaled = pd.DataFrame(
        data_test_numerical_scaled, columns=data_test_numerical.columns
    )

    # Encoding categorical data
    data_train_categorical_encoded = OE1.fit_transform(data_train_categorical)
    data_train_categorical_encoded = pd.DataFrame(
        data_train_categorical_encoded, columns=data_train_categorical.columns
    ).astype(int)
    data_test_categorical_encoded = OE1.transform(data_test_categorical)
    data_test_categorical_encoded = pd.DataFrame(
        data_test_categorical_encoded, columns=data_test_categorical.columns
    ).astype(int)

    # Encoding Ordinal data
    data_train_ordinal_encoded = OE2.fit_transform(data_train_ordinal)
    data_train_ordinal_encoded = pd.DataFrame(
        data_train_ordinal_encoded, columns=data_train_ordinal.columns
    ).astype(int)
    data_test_ordinal_encoded = OE2.transform(data_test_ordinal)
    data_test_ordinal_encoded = pd.DataFrame(
        data_test_ordinal_encoded, columns=data_test_ordinal.columns
    ).astype(int)
    ...
    dataset_train_normalized = pd.concat(
        [
            data_train_numerical_scaled,
            data_train_categorical_encoded,
            data_train_ordinal_encoded,
        ],
        axis=1,
    )
    dataset_test_normalized = pd.concat(
        [
            data_test_numerical_scaled,
            data_test_categorical_encoded,
            data_test_ordinal_encoded,
        ],
        axis=1,
    )
